...
float32 z"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      for val1 in self.homePos:
        _x = val1
        buff.write(_get_struct_3d().pack(_x.x, _x.y, _x.theta))
      _x = self.ballDetected
      buff.write(_get_struct_B().pack(_x))
      length = len(self.homeDetected)
      buff.write(_struct_I.pack(length))
      pattern = '<%sB'%length
      buff.write(struct.Struct(pattern).pack(*self.homeDetected))
      length = len(self.awayDetected)
      buff.write(_struct_I.pack(length))
      pattern = '<%sB'%length
      buff.write(struct.Struct(pattern).pack(*self.awayDetected))
      _x = self
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.homeDetected = s.unpack(str[start:end])
      self.homeDetected = list(map(bool, self.homeDetected))
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      pattern = '<%sB'%length
      ...
      self.awayDetected = s.unpack(str[start:end])
      self.awayDetected = list(map(bool, self.awayDetected))
      _x = self
      start = end
      end += 8
